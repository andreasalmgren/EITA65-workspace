...
def route_planner():
    ...
    if from_location is None:
        ...
        # If the coodinates are found by Nominatim, the coords will need to be sent to the a drone that is available
        ...
        while not found:
            i += 1
            found = redis_server.get('status' + str(i)) == 'idle'
            thing = redis_server.get('status' + str(i))
            if thing is None:
                break
        ...
        if not found:
            ...
            DRONE_IP = redis_server.get('droneIP' + str(i))
